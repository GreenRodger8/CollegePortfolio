...
%Final guess from newt1
ANS = 2.575402738307057e-01;
...
%Record iterates - a col vector of MAXIT length
cerrors = zeros(MAXIT,1);

%Record iterates - a col vector of MAXIT length
cerrors2 = zeros(MAXIT,1);
...
while abs(x-prev)/x >= TOL & count < MAXIT
    ...
    %Add new errors to lists of errors
    cerrors(count) = (abs(x-ANS))/abs(prev-ANS);
    cerrors2(count) = (abs(x-ANS))/((prev-ANS)^2);
