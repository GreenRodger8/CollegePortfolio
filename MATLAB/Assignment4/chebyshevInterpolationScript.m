...
%by interpolating n nodes using Chebyshov interpolation
...
%Add rest of Chebyshev x coordinates and their respective y coordinates
halfDistance = xCoordinates(n) - xCoordinates(1); 
for i=2:n-1
    xCoordinates(i) = halfDistance*(cos((2*i-1)*pi/(2*n))+1)+xCoordinates(1);
    yCoordinates(i) = exp(-1*abs(xCoordinates(i)));
end

%Compute coefficients for Chebyshov's polynomial
newtonCoefficients = newtdd(xCoordinates,yCoordinates,n);

%Compute values from Chebyshov's polynomial for evenly spaced x 
spacing = 0.01;
...
%Compute absolute error for Chebyshov's polynomial
...
maxError = (((xCoordinates(n)-xCoordinates(1))/n)^n)*maxDerivative/(factorial(n)*2^(n-1))
